...
export class BackendClient {
  ...
  async getPortfolioSnapshot(portfolioId: string, asOf?: string): Promise<TPortfolioSnapshot | null> {
    try {
  ...
  async getPortfolioReport(portfolioId: string, format: "json" | "csv" | "md" = "json"): Promise<any> {
    try {
  ...
  async getPositions(portfolioId: string, asOf?: string): Promise<TPosition[]> {
    try {
      // For now, use the reports API to get portfolio data
      const reportContent = await this.getPortfolioReport(portfolioId, "json");
      if (reportContent && reportContent.positions) {
        // Convert report positions to our Position schema
        return reportContent.positions.map((pos: any) => Position.parse({
  ...
  async getFactorExposures(portfolioId: string, asOf?: string): Promise<TPositionFactorExposure[]> {
    try {
      const params = new URLSearchParams();
      if (asOf) params.set("as_of", asOf);
      
      const endpoint = `/api/v1/portfolio/${portfolioId}/factors${params.toString() ? `?${params}` : ""}`;
      const response = await this.request<TPositionFactorExposure[]>(endpoint);
  ...
  async getRiskMetrics(portfolioId: string, asOf?: string): Promise<TRiskMetrics | null> {
    try {
      const params = new URLSearchParams();
      if (asOf) params.set("as_of", asOf);
      
      const endpoint = `/api/v1/portfolio/${portfolioId}/risk${params.toString() ? `?${params}` : ""}`;
      const response = await this.request<TRiskMetrics>(endpoint);
  ...
  async runStressTest(portfolioId: string, scenarios: string[]): Promise<TStressTestResult[]> {
    try {
      const endpoint = `/api/v1/portfolio/${portfolioId}/stress-test`;
      const response = await this.request<TStressTestResult[]>(endpoint, {
        method: "POST",
